...
const syncUserQuotaToKeysMock = vi.hoisted(() => vi.fn());
...
vi.mock("@/actions/users", () => ({
  ...
  syncUserQuotaToKeys: syncUserQuotaToKeysMock,
...
describe("v1 users endpoints", () => {
  beforeEach(() => {
    ...
    syncUserQuotaToKeysMock.mockResolvedValue(3);
  ...
  test("reads usage helpers and resets counters", async () => {
    ...
    const syncQuota = await callV1Route({
      method: "POST",
      pathname: "/api/v1/users/1/quota:sync-keys",
      headers,
    });
    expect(syncQuota.response.status).toBe(200);
    expect(syncQuota.json).toBe(3);
    expect(syncUserQuotaToKeysMock).toHaveBeenCalledWith(1);
  });
  ...
  test("documents user REST paths", async () => {
    ...
    expect(doc.paths).toHaveProperty("/api/v1/users/{id}/quota:sync-keys");
