...
import { userGroupConfigs } from "@/fork/portal/schema";
import type {
  UpsertUserGroupConfigInput,
  UserGroupConfig,
  UserGroupConfigKind,
} from "@/fork/portal/types/user-group-config";
...
export async function findAllUserGroupConfigs(): Promise<UserGroupConfig[]> {
  const rows = await db.select().from(userGroupConfigs).orderBy(asc(userGroupConfigs.groupName));
