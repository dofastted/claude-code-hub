...
// User Group Configs table - 控制用户分组上的业务能力
export const userGroupConfigs = pgTable('user_group_configs', {
  id: serial('id').primaryKey(),
...
// Portal Plans table - 套餐模板由 CCH 管理，供门户站读取
export const portalPlans = pgTable('portal_plans', {
  id: serial('id').primaryKey(),
...
// Portal User Links table - 门户用户与 CCH 用户/default key 的固定映射
export const portalUserLinks = pgTable('portal_user_links', {
  id: serial('id').primaryKey(),
...
// Portal Subscriptions table - 记录门户订单开通结果与限额快照
export const portalSubscriptions = pgTable('portal_subscriptions', {
  ...
  portalSubscriptionsCreatedAtIdx: index('idx_portal_subscriptions_created_at').on(table.createdAt),
}));

// Temporary Key Batches table - 批次级别记录临时 key 的来源和分组
export const temporaryKeyBatches = pgTable('temporary_key_batches', {
  id: serial('id').primaryKey(),
...
export const temporaryKeyBatchKeys = pgTable('temporary_key_batch_keys', {
  id: serial('id').primaryKey(),
